                                        ...
                                        <tr>
                                            ...
                                            <td style="text-align: center;">
                                                <span style="display: inline-flex; flex-direction: row; gap: 15px;">
                                                    ...
                                                    <form action="{{ route('approve.player') }}" method="POST">
                                                        @csrf

                                                        <input type="hidden" name="id" value="{{ $data->id }}">
                                                        <button class="btn btn-success btn-sm waves-effect btn-label waves-light" onclick="return confirm('Are you sure to approve the player ?');"> 
                                                            <i class="bx bx-check-double label-icon"></i> Approve
                                                        </button>
                                                    </form>

                                                    <form action="{{ route('decline.player') }}" method="POST">
                                                        @csrf

                                                        <input type="hidden" name="id" value="{{ $data->id }}">
                                                        <button class="btn btn-danger btn-sm waves-effect btn-label waves-light" onclick="return confirm('Are you sure to decline and delete permanently ?');"> 
